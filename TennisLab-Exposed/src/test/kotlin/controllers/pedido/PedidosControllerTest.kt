...
class PedidosControllerTest {
    ...
    @Test
    fun getByIdNoExiste() {
        every { pedidosRepository.findById(pedido.id) } throws PedidoException("Pedido con id ${pedido.id} no existe")

        val res = assertThrows<PedidoException> { pedidosController.getPedido(pedido.id) }
    ...
    @Test
    fun deleteNoExiste() {
        every { pedidosRepository.delete(pedido) } throws PedidoException("Pedido con id ${pedido.id} no existe")

        val res = assertThrows<PedidoException> { pedidosController.deletePedido(pedido) }
